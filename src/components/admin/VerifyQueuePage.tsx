...
export default function VerifyQueuePage({ kind }: { kind: "user" | "seller" }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-auto rounded-2xl border border-emerald-100 bg-white shadow-sm">
        <table className="w-full text-sm text-slate-900">
          ...
          <tbody>
              ...
              >
                ...
                <td className="p-3 text-right">
                  <div className="flex gap-2 justify-end">
                    ...
                    <button
                      className="rounded-xl border border-red-300 bg-white px-3 py-2 text-sm font-semibold text-red-700 hover:bg-red-50"
            ...
            {!rows.length && !loading && (
              <tr>
                <td colSpan={4} className="p-6 text-center text-emerald-800/70">
                  No pending requests.
                </td>
              </tr>
            )}
...
function UserDetails({ data }: { data?: any }) {
  if (!data) {
    return <span className="text-emerald-800/70">No data.</span>;
  }
  ...
  return (
    <div className="space-y-2 text-xs leading-relaxed">
      <div>
        ...
        {email || <span className="text-emerald-800/70">-</span>}
      </div>
      <div>
        <span className="font-semibold">Cognito address:</span>{" "}
        {address || <span className="text-emerald-800/70">-</span>}
      </div>
      <div>
        <span className="font-semibold">IP:</span>{" "}
        {ip || <span className="text-emerald-800/70">-</span>}
      </div>
...
function SellerDetails({ seller }: { seller?: any }) {
  if (!seller) {
    return <span className="text-emerald-800/70">No seller profile.</span>;
  }
  ...
  return (
    <div className="space-y-2 text-xs leading-relaxed">
      <div>
        ...
        {seller.organisationName || <span className="text-emerald-800/70">-</span>}
      </div>
      <div>
        <span className="font-semibold">Address:</span>{" "}
        {seller.address || <span className="text-emerald-800/70">-</span>}
      </div>
      <div>
        <span className="font-semibold">Email:</span>{" "}
        {seller.contactEmail || <span className="text-emerald-800/70">-</span>}
      </div>
      <div>
        <span className="font-semibold">Phone:</span>{" "}
        {seller.contactPhone || <span className="text-emerald-800/70">-</span>}
      </div>
